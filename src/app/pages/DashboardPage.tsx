...
export function DashboardPage() {
  ...
  const pcd = useMemo(() => {
    ...
    return data.progress
      .filter(
        (p) =>
          p.studentId === Number(ps) && p.date >= psStart && p.date <= psEnd,
      )
      .sort((a, b) => a.date.localeCompare(b.date))
      .map((p) => ({
        date: p.date,
        "Kuran Sayfa": p.kuranCurrentPage,
        "Risale Sayfa": p.risaleCurrentPage,
        "Elif-ba Sayfa": p.elifbaCurrentPage,
      }));
  ...
  const stats = [
    ...
    {
      title: "Bugün Mevcut",
      value: todayAtt.filter((a) => a.status === "present").length,
      icon: CheckCircle2,
      color: "bg-green-500",
    },
    {
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="p-4 bg-amber-50 border border-amber-200 rounded-lg flex items-start gap-3">
          ...
          <div>
            <p className="font-medium text-amber-800 text-sm">
              Yoklama Hatırlatması
            </p>
            <p className="text-xs text-amber-700 mt-1">
              Bugün henüz yoklama almadınız. Öğrencilerin devam durumunu
              kaydetmek için{" "}
              <Link to="/attendance" className="underline font-medium">
                Yoklama
              </Link>{" "}
              sayfasına gidin.
            </p>
          ...
          <Card key={i} className="hover:shadow-md transition-shadow">
            <CardContent className="p-4 sm:p-6">
              <div className="flex items-center justify-between">
                ...
                <div className={`${s.color} p-2 sm:p-3 rounded-lg text-white`}>
      ...
      <Card>
        <CardHeader className="pb-3">
          <CardTitle className="text-base flex items-center gap-2">
            <TrendingUp size={18} /> Öğrenci Öğrenme İlerlemesi
          </CardTitle>
        </CardHeader>
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            ...
            <div className="w-full sm:w-40">
              <Input
                type="date"
                value={psStart}
                onChange={(e) => setPsStart(e.target.value)}
              />
            </div>
            <div className="w-full sm:w-40">
              <Input
                type="date"
                value={psEnd}
                onChange={(e) => setPsEnd(e.target.value)}
              />
            </div>
          </div>
          {ps ? (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <Card>
          <CardHeader className="pb-3">
            <CardTitle className="text-base">Son Öğrenciler</CardTitle>
          </CardHeader>
          <CardContent>
                    ...
                    >
                      <div>
                        <p className="font-medium text-sm">
                          ...
                          {canViewColumn("students", "tcKimlik") && (
        ...
        <Card>
          <CardHeader className="pb-3">
            <CardTitle className="text-base">Son Gelişim</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="space-y-3">
              {data.progress
                  ...
                  return (
                    <div key={p.id} className="p-3 bg-gray-50 rounded-lg">
                      ...
                      <div className="flex flex-wrap gap-2 mt-1">
                        <Badge variant="outline" className="text-xs">
                          Kuran S.{p.kuranCurrentPage}
                        </Badge>
                        <Badge variant="outline" className="text-xs">
                          Risale S.{p.risaleCurrentPage}
                        </Badge>
                        {p.elifbaCurrentPage > 0 && (
                          <Badge variant="outline" className="text-xs">
                            Elif-ba S.{p.elifbaCurrentPage}
                          </Badge>
                        )}
                      </div>
                    </div>
