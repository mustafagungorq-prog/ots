...
export function ReportsPage() {
  ...
  const generateReport = () => {
    ...
    const content =
      `SAYIN ${st.parentName},\n\n${st.firstName} ${st.lastName} için ${pt}:\n\nÖĞRENCİ:\n- Ad: ${st.firstName} ${st.lastName}\n- Sınıf: ${st.grade}\n- Okul: ${data.schools.find((s) => s.id === st.schoolId)?.name || "-"}\n- Dersler: ${sl.map((l) => l.name).join(", ")}\n\nKURAN:\n${lp ? `- Son: ${lp.kuranCurrentPage}\n- Okunan: ${lp.kuranPages}` : "- Yok"}\n\nRİSALE:\n${lp ? `- Son: ${lp.risaleCurrentPage}\n- Okunan: ${lp.risalePages}` : "- Yok"}\n\nELİF-BA:\n${lp ? `- Son: ${lp.elifbaCurrentPage}` : "- Yok"}\n\nYOKLAMA:\n- Toplam: ${sa.length}, Mevcut: ${sa.filter((a) => a.status === "present").length}, İzinli: ${sa.filter((a) => a.status === "excused").length}, Geç: ${sa.filter((a) => a.status === "late").length}, Yok: ${sa.filter((a) => a.status === "absent").length}\n\nYORUMLAR:\n${
        data.comments
          .filter((c) => c.studentId === st.id && c.type === "teacher")
          .slice(-3)
  ...
  const handleSend = async () => {
    ...
    const sentViaForApi =
      sendVia.includes("email") && sendVia.includes("sms")
        ? "both"
    ...
    try {
      if (sendVia.includes("email") && st.email) {
        const subject = `${st.firstName} ${st.lastName} - Gelişim Raporu`;
        const html = `<html><body style="font-family:Arial,Helvetica,sans-serif;white-space:pre-wrap;">${preview.replace(/\n/g, "<br>")}</body></html>`;
        await sendMailViaPhp({ to: st.email, subject, html, text: preview });
      }
      if (sendVia.includes("sms")) {
        const text = encodeURIComponent(preview);
        window.open(
          `sms:${st.parentPhone || st.phone || ""}?body=${text}`,
          "_blank",
        );
      }
      data.addReport({
        studentId: Number(selStudent),
        type: reportType,
        periodStart: start,
        periodEnd: now,
        content: preview,
        sentVia: sentViaForApi,
      });
      setShowPreview(false);
      setPreview("");
      alert("Rapor gönderildi.");
    } catch (err: any) {
      alert(`Gönderim sırasında hata oluştu: ${err?.message || "hata"}`);
    }
  };
  const toggleSendVia = (m: string) =>
    setSendVia((p) => (p.includes(m) ? p.filter((x) => x !== m) : [...p, m]));
  ...
  const getReportDate = (r: any) =>
    r?.sentAt || r?.createdAt || r?.created_at || "-";
  /*
  if (
    data.loadingStudents || data.loadingLessons || data.loadingProgress ||
    data.loadingAttendance || data.loadingSchools || data.loadingComments ||
    data.loadingReports
  ) {
    return <Loading />;
  }*/
    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader className="pb-3">
          <CardTitle className="text-base flex items-center gap-2">
            <FileText size={18} /> Gelişim Raporu
          </CardTitle>
        </CardHeader>
        <CardContent className="space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            ...
            <div className="space-y-1">
              <Label className="text-xs">Gönderim</Label>
      ...
      {showPreview && (
        <Card>
          <CardHeader>
            <CardTitle className="text-base flex items-center gap-2">
              <Send size={16} /> Önizleme
            </CardTitle>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="bg-gray-50 p-4 rounded-lg border whitespace-pre-line text-sm leading-relaxed">
              {preview}
            </div>
      ...
      <Card>
        <CardHeader className="pb-3">
          <CardTitle className="text-base">Gönderilen Raporlar</CardTitle>
        </CardHeader>
        <CardContent className="p-0 overflow-x-auto">
          <Table>
            <TableHeader>
              <TableRow>
                {canViewColumn("reports", "date") && (
                  <TableHead className="text-xs">Tarih</TableHead>
                )}
                {canViewColumn("reports", "student") && (
                  <TableHead className="text-xs">Öğrenci</TableHead>
                )}
                {canViewColumn("reports", "type") && (
                  <TableHead className="text-xs">Dönem</TableHead>
                )}
                {canViewColumn("reports", "method") && (
                  <TableHead className="text-xs">Yöntem</TableHead>
                )}
                {canViewColumn("reports", "status") && (
                  <TableHead className="text-xs">Durum</TableHead>
                )}
                {canViewColumn("reports", "actions") && (
                    ...
                    <TableRow key={r.id}>
                      ...
                      {canViewColumn("reports", "status") && (
                        <TableCell>
                          <Badge
                            variant={
                              r.status === "sent" ? "default" : "outline"
                            }
                            className="text-xs"
                          >
                            {r.status === "sent" ? "Gönderildi" : "Taslak"}
                          </Badge>
                        </TableCell>
                      )}
                      {canViewColumn("reports", "actions") && (
                        <TableCell>
                          <Button
                            ...
                            size="icon"
                            className="h-8 w-8"
                            onClick={() => data.deleteReport(r.id)}
                          >
