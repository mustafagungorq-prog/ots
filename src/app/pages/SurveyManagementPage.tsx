...
export function SurveyManagementPage() {
  ...
  const [questionForm, setQuestionForm] = useState<Partial<SurveyQuestion>>({
    type: "text",
    options: [],
    sortOrder: 0,
  });
  ...
  /*
  if (data.loadingSurveys || data.loadingSurveyQuestions) {
    return <Loading />;
  }*/

  const selectedSurvey = surveys.find((s) => String(s.id) === selectedSurveyId);
  const questions = selectedSurveyId
    ? getSurveyQuestions(Number(selectedSurveyId))
    : [];
  ...
  const handleSurveySubmit = () => {
    ...
    if (editingSurvey) updateSurvey(editingSurvey.id, surveyForm);
      ...
      const newSurvey = addSurvey(
        surveyForm as Omit<Survey, "id" | "createdAt">,
      );
      if (newSurvey) setSelectedSurveyId(String(newSurvey.id));
    }
    setOpen(false);
    setEditingSurvey(null);
    setSurveyForm({ active: true });
  };
  const handleQuestionSubmit = () => {
    if (!questionForm.questionText || !questionForm.type || !selectedSurveyId)
      return;
    addSurveyQuestion({
      ...(questionForm as Omit<SurveyQuestion, "id">),
      surveyId: Number(selectedSurveyId),
      sortOrder: getSurveyQuestions(Number(selectedSurveyId)).length,
    });
    setQuestionForm({ type: "text", options: [], sortOrder: 0 });
    setOptionInput("");
  };
  const addOption = () => {
  ...
  const removeOption = (idx: number) => {
    setQuestionForm((f) => ({
      ...f,
      options: (f.options || []).filter((_: string, i: number) => i !== idx),
    }));
  };
  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-between items-center">
        ...
        >
          <Plus size={18} className="mr-1" /> Anket Ekle
        </Button>
      </div>
          ...
          <div className="flex flex-col sm:flex-row gap-3 items-start sm:items-center">
            ...
            {selectedSurvey && (
              <div className="flex gap-2">
                ...
                >
                  <Trash2 size={14} className="mr-1 text-red-500" /> Sil
                </Button>
              </div>
            )}
          </div>
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <CardDescription className="text-xs">
                  {selectedSurvey.description || "Açıklama yok"} •{" "}
                  {questions.length} soru •{" "}
                  <Badge
                    variant={selectedSurvey.active ? "default" : "secondary"}
                    className="text-[10px]"
                  >
                    {selectedSurvey.active ? "Aktif" : "Pasif"}
                  </Badge>
                </CardDescription>
              </div>
            </div>
            ...
            <div className="p-4 bg-emerald-50 rounded-lg space-y-3">
              ...
              <div className="space-y-1">
                ...
                >
                  ...
                  <SelectContent>
                    <SelectItem value="text">Metin (Text)</SelectItem>
                    <SelectItem value="select">Tek Seçim (Select)</SelectItem>
                    <SelectItem value="multiselect">
                      Çok Seçim (Multi Select)
                    </SelectItem>
                    <SelectItem value="test">Test (Doğru/Yanlış)</SelectItem>
                  </SelectContent>
                </Select>
              </div>
      ...
      {surveys.length === 0 && (
        <p className="text-center py-8 text-gray-500">
          Henüz anket yok. "Anket Ekle" butonu ile yeni anket oluşturun.
        </p>
      )}
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="max-w-lg">
          ...
          <div className="space-y-3 pt-4">
            <div className="space-y-1">
              <Label className="text-xs">Anket Başlığı *</Label>
              <Input
                value={surveyForm.title || ""}
                onChange={(e) =>
                  setSurveyForm((f) => ({ ...f, title: e.target.value }))
                }
              />
            </div>
            <div className="space-y-1">
              ...
              <Textarea
                ...
                onChange={(e) =>
                  setSurveyForm((f) => ({ ...f, description: e.target.value }))
                }
              />
            </div>
            <div className="flex items-center gap-2">
              <Switch
                checked={surveyForm.active ?? true}
                onCheckedChange={(v) =>
                  setSurveyForm((f) => ({ ...f, active: v }))
                }
              />
              <Label className="text-xs">Aktif</Label>
            </div>
            <Button onClick={handleSurveySubmit} className="w-full">
              {editingSurvey ? "Güncelle" : "Ekle"}
            </Button>
          </div>
        </DialogContent>
      </Dialog>
