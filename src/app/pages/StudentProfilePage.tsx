...
export function StudentProfilePage() {
  ...
  const [reportItems, setReportItems] = useState<
    Record<
      string,
      {
        subject: string;
        note: string;
        status: "iyi" | "orta" | "gelisim" | "baslangic";
        score: string;
      }
    >
  >({});
  ...
  const homeworks = data
    ...
    .sort((a, b) =>
      ((b.createdAt || b.created_at || "") as string).localeCompare(
  ...
  const buildReportMailBody = () => {
    ...
    return [
      ...
      "KURAN:",
      latestProgress
        ? `- Son: ${latestProgress.kuranCurrentPage}\n- Okunan: ${latestProgress.kuranPages}`
        : "- Yok",
      "",
      "RİSALE:",
      latestProgress
        ? `- Son: ${latestProgress.risaleCurrentPage}\n- Okunan: ${latestProgress.risalePages}`
        : "- Yok",
      "",
      "ELİF-BA:",
    ...
    <div className="max-w-5xl mx-auto space-y-4 min-w-0 overflow-x-hidden">
          ...
          <div className="flex items-start gap-4">
            <div className="flex-1">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-sm">
                <div>
                  <span className="text-gray-500 text-xs">Doğum Yılı</span>
                  <p className="font-medium">{student.birthYear || "-"}</p>
                </div>
                <div>
                  <span className="text-gray-500 text-xs">Yaş</span>
                  <p className="font-medium">{student.age || "-"}</p>
                </div>
                <div>
                  <span className="text-gray-500 text-xs">Memleket</span>
                ...
                <div>
                  <span className="text-gray-500 text-xs">Veli</span>
                  <p className="font-medium">{student.parentName || "-"}</p>
                </div>
                <div>
                  <span className="text-gray-500 text-xs">Veli Tel</span>
                ...
                <div>
                  <span className="text-gray-500 text-xs">Dersler</span>
                  <p className="font-medium">
                    {studentLessons.map((l) => l.name).join(", ") || "-"}
                  </p>
                </div>
              </div>
              {/* Mini yoklama ozeti */}
              <div className="flex gap-4 mt-3 pt-3 border-t">
                ...
                <div className="text-center">
                  ...
                  <p className="text-[10px] text-gray-500">Yok</p>
                </div>
                <div className="text-center">
                  <p className="text-xl font-bold text-blue-600">
                    {studentProgress.length}
                  </p>
                  <p className="text-[10px] text-gray-500">Gelişim</p>
                </div>
                <div className="text-center">
                  <p className="text-xl font-bold text-purple-600">
                    {homeworks.length}
                  </p>
                  <p className="text-[10px] text-gray-500">Ödev</p>
                </div>
                <div className="text-center">
                  <p className="text-xl font-bold text-emerald-600">
                    {lessonLogs.length}
                  </p>
                  <p className="text-[10px] text-gray-500">Ders</p>
                </div>
              </div>
            </div>
          </div>
      ...
      <div className="flex border-b bg-white rounded-t-lg px-4 pt-2 yasar-fix">
        ...
        <button
          ...
          className={`px-4 py-2 text-sm font-medium border-b-2 transition-colors ${activeTab === "attendance" ? "border-emerald-600 text-emerald-700" : "border-transparent text-gray-500"}`}
        >
        ...
        <button
          ...
          className={`px-4 py-2 text-sm font-medium border-b-2 transition-colors ${activeTab === "progress" ? "border-emerald-600 text-emerald-700" : "border-transparent text-gray-500"}`}
        >
        ...
        <button
          ...
          className={`px-4 py-2 text-sm font-medium border-b-2 transition-colors ${activeTab === "homework" ? "border-emerald-600 text-emerald-700" : "border-transparent text-gray-500"}`}
        >
        ...
        <button
          ...
          className={`px-4 py-2 text-sm font-medium border-b-2 transition-colors ${activeTab === "lessonlog" ? "border-emerald-600 text-emerald-700" : "border-transparent text-gray-500"}`}
        >
          <NotebookPen size={16} className="inline mr-1" />
          Ders İşleme ({lessonLogs.length})
        </button>
        <button
          onClick={() => setActiveTab("report")}
          className={`px-4 py-2 text-sm font-medium border-b-2 transition-colors ${activeTab === "report" ? "border-emerald-600 text-emerald-700" : "border-transparent text-gray-500"}`}
        >
      ...
      {activeTab === "info" && (
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <Card>
            ...
            <CardContent className="space-y-2 text-sm">
              <div className="flex justify-between">
                ...
                <span className="font-medium">{student.tcKimlik || "-"}</span>
              </div>
              <div className="flex justify-between">
              ...
              <div className="flex justify-between">
                <span className="text-gray-500">Doğum Yılı</span>
                <span className="font-medium">{student.birthYear || "-"}</span>
              </div>
              <div className="flex justify-between">
              ...
              <div className="flex justify-between">
                <span className="text-gray-500">Memleket</span>
          ...
          <Card>
            ...
            <CardContent className="space-y-2 text-sm">
              <div className="flex justify-between">
                <span className="text-gray-500">Öğrenci Tel</span>
              ...
              <div className="flex justify-between">
                <span className="text-gray-500">Veli Tel</span>
                <span className="font-medium">
                  {student.parentPhone || "-"}
                </span>
              </div>
          ...
          <Card>
            <CardHeader className="pb-2">
              <CardTitle className="text-sm">Eğitim</CardTitle>
            </CardHeader>
            <CardContent className="space-y-2 text-sm">
              <div className="flex justify-between">
        ...
        <Card>
          <CardHeader className="pb-3">
            <CardTitle className="text-base">
              Yoklama Geçmişi — {studentAttendance.length} kayıt
            </CardTitle>
          </CardHeader>
      ...
      {activeTab === "progress" && (
        <Card>
          <CardHeader className="pb-3">
            <CardTitle className="text-base">
              Öğrenim Durumu — {studentProgress.length} kayıt
            </CardTitle>
          </CardHeader>
          <CardContent className="p-0 overflow-x-auto max-h-[500px] overflow-y-auto">
                  ...
                  {studentProgress.map((p) => (
                    <TableRow key={p.id}>
                      ...
                      <TableCell className="text-xs text-center">
                        <Badge
                          variant="outline"
                          className="text-green-700 border-green-300 text-[10px]"
                        >
                      ...
                      <TableCell className="text-xs text-center">
                        <Badge
                          variant="outline"
                          className="text-purple-700 border-purple-300 text-[10px]"
                        >
                      ...
                      <TableCell className="text-xs text-center">
                        <Badge
                          variant="outline"
                          className="text-orange-700 border-orange-300 text-[10px]"
                        >
      ...
      {activeTab === "lessonlog" && (
        <Card>
          <CardHeader className="pb-3">
            <CardTitle className="text-base">
              Ders İşleme Kayıtları — {lessonLogs.length} kayıt
            </CardTitle>
          </CardHeader>
          <CardContent className="p-0 overflow-x-auto max-h-[500px] overflow-y-auto">
            {lessonLogs.length > 0 ? (
      ...
      {/* --- RAPOR TAB --- */}
      {activeTab === "report" && (
        <div className="space-y-4 w-full max-w-full min-w-0 overflow-x-hidden overflow-y-hidden">
          <div className="space-y-4 w-full min-w-0 pr-0 sm:pr-4">
                  ...
                  <Button
                    ...
                    onClick={() => {
                      ...
                      studentLessons.forEach((l) => {
                        ...
                        const lessonProgress = data.progress
                          .filter((p) => p.studentId === student.id)
                          .sort((a, b) => b.date.localeCompare(a.date))[0];
                        let subject = l.name;
                        let note = lessonProgress
                          ? `Son kayıt: ${lessonProgress.date} - Kuran S.${lessonProgress.kuranCurrentPage}, Risale S.${lessonProgress.risaleCurrentPage}`
                          : "Henüz kayıt yok";
                        let status: "iyi" | "orta" | "gelisim" | "baslangic" =
                          "baslangic";
                        if (
                          lessonProgress &&
                          lessonProgress.kuranCurrentPage > 50
                        )
                          status = "iyi";
                        else if (
                          lessonProgress &&
                          lessonProgress.kuranCurrentPage > 20
                        )
                          status = "orta";
                  ...
                  <Button
                    ...
                    onClick={() => {
                      const text = encodeURIComponent(
                        `365 Kuran Kuran Mektebi - ${student.firstName} ${student.lastName} Gelişim Raporu. Medrese: ${school?.name || "-"}. Detaylı bilgi için bizi arayabilirsiniz.`,
                      );
                      window.open(
            ...
            <Card className="print:shadow-none w-full min-w-0">
              <CardContent className="p-6 space-y-6 w-full min-w-0">
                  ...
                  <>
                    <div className="w-full min-w-0">
                      <h4 className="font-bold text-sm mb-3 flex items-center gap-2">
                        <FileText size={16} className="text-emerald-600" /> Ders
                        Bazlı Değerlendirmeler
                      </h4>
                          ...
                          .map((item, idx) => (
                            ...
                            >
                              <div className="flex items-start justify-between gap-2">
                                ...
                                <Badge
                                  className={`text-[10px] text-white ${item.status === "iyi" ? "bg-green-500" : item.status === "orta" ? "bg-blue-500" : item.status === "gelisim" ? "bg-yellow-500" : "bg-gray-500"}`}
                                >
                    ...
                    {studentReportsList.filter((r) => r.generalNote).length >
                      0 && (
                      <div>
                            ...
                            .map((r) => (
                              ...
                              >
                                <span className="text-xs text-gray-400">
                                  {r.createdAt} • {r.period}
                                </span>
                                <p className="mt-1">{r.generalNote}</p>
                              </div>
                            ))}
                        </div>
                      </div>
                          ...
                          >
                            ...
                            <div className="grid grid-cols-3 gap-2 text-[10px]">
                              <div className="rounded bg-green-50 p-2 text-center">
                                <div className="text-gray-500">Kuran</div>
                                <div className="font-semibold text-green-700">
                                  +{p.kuranPages}/S.{p.kuranCurrentPage}
                                </div>
                              </div>
                              <div className="rounded bg-purple-50 p-2 text-center">
                                <div className="text-gray-500">Risale</div>
                                <div className="font-semibold text-purple-700">
                                  +{p.risalePages}/S.{p.risaleCurrentPage}
                                </div>
                              </div>
                              <div className="rounded bg-orange-50 p-2 text-center">
                                <div className="text-gray-500">Elif-ba</div>
                  ...
                  {studentAttendance.length > 0 ? (
                    <div className="flex flex-wrap gap-2">
                      {studentAttendance.slice(0, 30).map((a) => (
                        <div
                          key={a.id}
                          className={`w-8 h-8 rounded flex items-center justify-center text-[10px] font-bold text-white ${a.status === "present" ? "bg-green-500" : a.status === "absent" ? "bg-red-500" : a.status === "late" ? "bg-yellow-500" : "bg-blue-500"}`}
                          title={`${a.date} - ${statusL[a.status]}`}
                        >
                          {a.date.slice(8)}
                        </div>
                      ))}
                ...
                <div className="w-full min-w-0">
                  <h4 className="font-bold text-sm mb-3 flex items-center gap-2">
                    <BookOpen size={16} className="text-emerald-600" /> Ödev
                    Durumu
                  </h4>
                  {homeworks.length > 0 ? (
                    <div className="flex flex-wrap gap-2">
            ...
            {canManageReports && (
              <Dialog open={reportOpen} onOpenChange={setReportOpen}>
                <DialogContent className="max-w-2xl max-h-[85vh] overflow-y-auto w-[95vw] sm:w-full">
                  <DialogHeader>
                    <DialogTitle>Öğrenci Raporu Ekle</DialogTitle>
                  </DialogHeader>
                  <div className="space-y-3 pt-2">
                        ...
                        return (
                          <Card key={l.id} className="p-3">
                              ...
                              <Select
                                ...
                                onValueChange={(v) =>
                                  setReportItems((prev) => ({
                                    ...
                                    [key]: {
                                      ...item,
                                      status: v as typeof item.status,
                                    },
                                  }))
                                }
                              >
                                ...
                                <SelectContent>
                                  ...
                                  <SelectItem value="gelisim">
                                    Gelişim
                                  </SelectItem>
                                  <SelectItem value="baslangic">
                                    Başlangıç
                                  </SelectItem>
                                </SelectContent>
                              </Select>
                            ...
                            <Input
                              ...
                              placeholder="Konu / Alt ders"
                              value={item.subject}
                              onChange={(e) =>
                                setReportItems((prev) => ({
                                  ...prev,
                                  [key]: { ...item, subject: e.target.value },
                                }))
                              }
                            />
                            <Textarea
                    ...
                    <div className="space-y-1 w-full min-w-0">
                      <Label className="text-xs">Genel Değerlendirme</Label>
                      <Textarea
                        value={reportNote}
                        onChange={(e) => setReportNote(e.target.value)}
                        placeholder="Öğrenci hakkında genel değerlendirme..."
                        rows={3}
                      />
                    </div>
                    <div className="flex gap-2">
                      <Button
                        ...
                        className="flex-1"
                      >
                        <Save size={16} className="mr-1" /> Rapor Ekle
                      </Button>
                      <Button
                        variant="outline"
        ...
        <div className="space-y-3">
          {homeworks.length > 0 ? (
            homeworks.map((h) => (
              <Card
                key={h.id}
                className={h.completed ? "border-green-300 bg-green-50/30" : ""}
              >
                <CardContent className="p-4">
                  <div className="flex items-start gap-3">
                    ...
                    <div className="pt-0.5">
                      <button
                        type="button"
                        onClick={() => data.toggleHomeworkCompleted(h.id)}
                        className={`w-6 h-6 rounded-full flex items-center justify-center transition-colors ${h.completed ? "bg-green-500 text-white" : "border-2 border-gray-300 hover:border-green-400"}`}
                        title={h.completed ? "Tamamlandı" : "Yapılmadı"}
                      >
