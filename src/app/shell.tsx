...
export function MainLayout({ children }: { children: ReactNode }) {
  ...
  const allTabs = [
    ...
    { id: 'memorization-tracking', label: 'Ezber Takip', icon: ListChecks, path: '/memorization-tracking', roles: PERMISSIONS.MEMORIZATION_TRACK },
    { id: 'memorization-texts-admin', label: 'Ezber Metin Yönetimi', icon: ListChecks, path: '/memorization-texts-admin', roles: ['superadmin', 'admin'] as const },
    { id: 'users', label: 'Kullanıcılar', icon: UserCog, path: '/users', roles: PERMISSIONS.USER_MANAGE },
  ];

  const parentLinkedStudentId = currentUser?.role === 'parent' ? currentUser.linkedStudentIds?.[0] : undefined;
  const visibleTabs = currentUser?.role === 'parent'
    ? (parentLinkedStudentId
      ? [{ id: 'my-student', label: 'Öğrenci Profili', icon: Users, path: `/student-profile/${parentLinkedStudentId}`, roles: ['parent'] as const }]
      : [])
  ...
  useEffect(() => {
    ...
    const frame = window.requestAnimationFrame(() => {
      const activeItem = mobileNavRef.current?.querySelector('[data-active="true"]') as HTMLElement | null;
        ...
        {sidebar.mobileOpen && (
          <div className="fixed inset-0 z-50 lg:hidden">
            ...
            <aside className="absolute left-0 top-0 bottom-0 w-64 bg-white shadow-xl flex flex-col">
              ...
              <nav ref={mobileNavRef} className="flex-1 overflow-y-auto overscroll-contain p-2 space-y-1">
                {visibleTabs.map(tab => (
                  <button
                    key={tab.id}
                    onClick={() => handleNav(tab.path)}
