...
-- New: passed    | failed | repeat_tecvid | repeat_harf

-- 1. Expand enum to hold both old and new values temporarily.
ALTER TABLE memorization_tracking
  MODIFY COLUMN status ENUM('completed','repeat','not_completed','passed','failed','repeat_tecvid','repeat_harf')
...
-- 3. Reduce enum to the final new values.
ALTER TABLE memorization_tracking
  MODIFY COLUMN status ENUM('passed','failed','repeat_tecvid','repeat_harf')
